...
from vvalgo.graph import Graph, WeightedGraph
...
def test_graph_from_file_weighted(graph_file_weighted, graph_adj_list):
    graph = WeightedGraph.from_file(graph_file_weighted)
    assert graph.max_vertex == 4
    assert graph.adj_list == {k: list(map(lambda x: (x, i), values))
                              for i, (k, values) in
                              enumerate(graph_adj_list.items(), 1)}